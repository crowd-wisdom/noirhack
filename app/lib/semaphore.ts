...
export async function addIdentityToCuratorsGroup() {
    ...
    const proof: SemaphoreNoirProof = await generateNoirProof(identity, group, "add member to curator group", group.root)
    console.log("identity to group proof", proof)
    console.log("saveMemberOnChain ...", proof)
    const response_tx=await saveMemberOnChain(SEMAPHORE_CURATOR_GROUP_ID,identity.commitment)
    console.log("saveMemberOnChain ... successfull. Status of tx", response_tx.status)
    return proof;
}

const saveMemberOnChain = async (idGroup:string,identityCommitment:bigint) : Promise<TransactionReceipt>  => {
